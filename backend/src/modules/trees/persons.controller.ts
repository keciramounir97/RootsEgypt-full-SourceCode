...
export class PersonsController {
  ...
  private async ensureTreeAccess(treeId: number, user: any) {
    ...
    const canManageAll =
      user.roleName === "admin" || user.roleName === "super_admin"; // Simplified role check
  ...
  async getPublicPerson(@Param("id", ParseIntPipe) id: number) {
    const person = await Person.query(this.knex)
      .findById(id)
  ...
  async updateMyPerson(
    ...
    const updated = await Person.query(this.knex).patchAndFetchById(id, {
      name: body.name,
    });
