...
async function ensureCriticalSchema(knex) {
    try {
        ...
        if ((await knex.schema.hasTable("family_trees")) &&
            ...
            await knex.schema.alterTable("family_trees", (t) => t.text("gedcom_text", "longtext").nullable());
        ...
        if (await knex.schema.hasTable("gallery")) {
            if (!(await knex.schema.hasColumn("gallery", "seed_key"))) {
                await knex.schema.alterTable("gallery", (t) => t.string("seed_key", 120).nullable());
                console.log("Schema patch: added gallery.seed_key");
            }
            if (!(await knex.schema.hasColumn("gallery", "show_details"))) {
                await knex.schema.alterTable("gallery", (t) => t.boolean("show_details").defaultTo(true).nullable());
    ...
    try {
        ...
        app.use(async (req, res, next) => {
            ...
            try {
                ...
                const storedGedcom = (0, trees_controller_1.getStoredGedcomText)(tree);
                if (storedGedcom) {
