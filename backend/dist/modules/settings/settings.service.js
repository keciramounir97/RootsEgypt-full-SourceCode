...
let SettingsService = class SettingsService {
    ...
    parseValue(key, value) {
        ...
        if (key === 'defaultLanguage') {
            return ['en', 'fr', 'ar', 'es'].includes(value) ? value : DEFAULT_SETTINGS.defaultLanguage;
        }
        return value;
    }
    ...
    async readSetting(key, fallback) {
        ...
        return (row === null || row === void 0 ? void 0 : row.value) !== undefined && (row === null || row === void 0 ? void 0 : row.value) !== null ? String(row.value) : fallback;
    }
    async writeSetting(key, value) {
        ...
        if (existing) {
            await this.knex('app_settings').where({ key }).update({ value: serialized, updated_at: now });
        }
        else {
            await this.knex('app_settings').insert({ key, value: serialized, updated_at: now });
        }
    }
    formatSiteImage(row) {
        return {
    ...
    async updateSettings(data, actorId) {
        ...
        for (const key of SETTING_KEYS) {
            ...
            const existing = await this.knex('app_settings').where({ key }).first();
            if (existing) {
                await this.knex('app_settings').where({ key }).update({ value, updated_at: now });
            }
            else {
    ...
    async getSiteImages() {
        ...
        const [heroDefault, backgroundDefault, heroRows, backgroundRows] = await Promise.all([
            ...
            this.knex('site_images').where({ type: 'hero' }).orderBy('sort_order', 'asc').orderBy('id', 'asc'),
            this.knex('site_images').where({ type: 'background' }).orderBy('sort_order', 'asc').orderBy('id', 'asc'),
        ]);
    ...
    async deleteHeroImage(id, actorId) {
        await this.ensureSettingsSchema();
        const row = await this.knex('site_images').where({ id, type: 'hero' }).first();
        if (!row)
            return this.getSiteImages();
    ...
    async deleteBackgroundImage(id, actorId) {
        await this.ensureSettingsSchema();
        const row = await this.knex('site_images').where({ id, type: 'background' }).first();
        if (!row)
            return this.getSiteImages();
        (0, file_utils_1.safeUnlink)((0, file_utils_1.resolveStoredFilePath)(row.image_path));
    ...
    async deleteBackgroundImages(actorId) {
        await this.ensureSettingsSchema();
        const existing = await this.knex('site_images').where({ type: 'background' }).select('id', 'image_path');
