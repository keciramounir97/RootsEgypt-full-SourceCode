...
let TreesService = class TreesService {
    ...
    inferDataFormat(filename, content) {
        ...
        if (ext === 'json' || ext === 'xml' || ext === 'gedx')
            return 'gedcomx';
        if ((ext === 'ged' || ext === 'gedcom') && content && /VERS\s+7\.0/i.test(content))
            return 'gedcom7';
        return 'gedcom';
    }
        ...
        try {
            ...
            if (format === 'json') {
                try {
                    ...
                    people = parsed.map((p) => ({
                        name: (p.names && p.names.en) || [p.given, p.surname].filter(Boolean).join(' ') || 'Unknown',
                    }));
                }
                catch (e) {
